...
# Workspace directory
workspace = "D:/ebird/Mexico/Analysis/Regional/"
...
for (sp in 1:length(birdlist)) {
  ...
  popobj <- read.csv(paste(workspace,"Mexico_regional.csv",sep="/"), sep=",", header=TRUE, quote = "", stringsAsFactors = FALSE, na.strings=c(""))
  setwd(workspace)
  #Setup harvest and bcr input
  popobj = subset(popobj, popobj$species == species)
  ...
  # Iterate through ebird data by BCR
    ...
    max = 0
    harvestclass = "4d"
    seasonsub = subset(ss$y, (as.Date(ss$x, format="%m/%d") >= as.Date("12/1", format="%m/%d") | (as.Date(ss$x, format="%m/%d")) <= as.Date("4/30", format="%m/%d")))
    ...
    newsub = subset(popobj, popobj$species == species)
